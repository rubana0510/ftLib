package com.feedbacktower.utilities.videotrimmer_kt.utils
...
import com.feedbacktower.utilities.videotrimmer_kt.compressor.VideoCompress
...
object TrimVideoUtils {
    ...
    fun startTrim(
        ...
        val file = File(context.getExternalFilesDir(null), "temp").apply {
            mkdirs()
        }

        val trimmedVideoFile = File(file.absolutePath, outputFinalVideoFile.name)

//        Log.d("AppLog", "startTrim")
        outputFinalVideoFile.parentFile.mkdirs()
        outputFinalVideoFile.delete()
        var succeeded = false
...
//            Log.d("AppLog", "trimmed file is the entire video, so just copy it")
            context.contentResolver.openInputStream(inputVideoUri).use {
                it?.copyTo(FileOutputStream(trimmedVideoFile))
                succeeded = it != null && trimmedVideoFile.exists()
            }
...
//            Log.d("AppLog", "trying to trim using mp4parser...")
            try {
                ...
                succeeded = genVideoUsingMp4Parser(inputFilePath, trimmedVideoFile, startMs, endMs)
            } catch (e: Exception) {
            }
...
//            Log.d("AppLog", "succeeded trimming using Android framework API?$succeeded")
        ...
        Handler(Looper.getMainLooper()).post {
            ...
            if (succeeded) {
                val uri = Uri.parse(trimmedVideoFile.absolutePath)
                callback.onFinishedTrimming(uri)
                if (compress) {
                    VideoCompress.compressVideoMedium(trimmedVideoFile.absolutePath, outputFinalVideoFile.absolutePath, object : VideoCompress.CompressListener{
                        override fun onStart() {
                            callback.onCompressStarted()
                        }
                        ...
                        override fun onProgress(percent: Float) {
                            callback.onCompressProgress(percent)
                        }

                    })
                }
            } else {
                callback.onFinishedTrimming(null)
            }
        }
    }
